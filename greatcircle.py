...
class GreatCircle(object):
    ...
    def __validate_degrees(self):
        ...
        """
        Check latitudes and longitudes are within valid ranges and that they are not None,
        setting the valid attribute accordingly.
        """

        self.valid = True

        if math.isnan(self.latitude1_degrees) or self.latitude1_degrees == None or self.latitude1_degrees < -90.0 or self.latitude1_degrees > 90.0:
            self.valid = False

        if math.isnan(self.longitude1_degrees) or self.longitude1_degrees == None or self.longitude1_degrees < -180.0 or self.longitude1_degrees > 180.0:
            self.valid = False

        if math.isnan(self.latitude2_degrees) or self.latitude2_degrees == None or self.latitude2_degrees < -90.0 or self.latitude2_degrees > 90.0:
            self.valid = False

        if math.isnan(self.latitude2_degrees) or self.longitude2_degrees == None or self.longitude2_degrees < -180.0 or self.longitude2_degrees > 180.0:
            self.valid = False
    ...
    def __calculate_central_angle(self):
        ...
        """
        Formula for calculating the central angle
        ...
        self.central_angle_radians = math.acos(math.sin(self.latitude1_radians) * math.sin(self.latitude2_radians)
                                               + math.cos(self.latitude1_radians) * math.cos(self.latitude2_radians)
